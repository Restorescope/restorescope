 ...
 *   - Logo (file upload to storage bucket 'branding-assets')
 *   - Color palette (5 colors: primary, primary-dark, primary-light,
 *     accent, accent-dark)
 *
 * On save, the BrandingProvider context is refreshed which re-applies CSS
 * variables to :root — the entire app re-themes immediately, no reload.
 *
 * "Restore defaults" reverts to original 1-800 WATER DAMAGE of North Dakota
 * branding.
 *
 * AI color extraction from uploaded logos comes in a follow-up update.
 */
export default function Branding() {
  ...
  async function uploadLogoIfNeeded() {
    if (!logoFile) return form.logo_path  // unchanged
        ...
        <Card>
          ...
          <CardBody className="space-y-3">
            ...
            <p className="text-xs text-ink-500">
              Coming soon: upload a logo and AI will detect the brand colors automatically.
            </p>
          </CardBody>
        </Card>
...
/**
 * BrandPreview — small inline preview of how the chosen colors look together
 * on a few common UI elements. Reads from the form state (live preview)
 * rather than the saved branding, so the Owner can see changes before saving.
 */
